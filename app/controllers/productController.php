...
class productController extends mainModel
{
    ...
    public function obtenerOpcionesAlmacenes()
    {
        ...
        while ($tipo_moneda = $datos_almacenes->fetch()) {
            $opciones_almacenes .= '<option value="' . $tipo_moneda['id_moneda'] . '">'
                . $tipo_moneda['nombre_moneda'] . '</option>';
        }
...
public function listarProductControlador($pagina, $registros, $url, $busqueda)
{
    ...
    $consulta_datos = "SELECT
            ...
            SUM(CASE WHEN almacenes.nombre_almacen = 'Almacén Maquinados' THEN stock_almacen.stock ELSE 0 END) AS stock_maquinados,
            SUM(CASE WHEN almacenes.nombre_almacen = 'Almacén Ensamble' THEN stock_almacen.stock ELSE 0 END) AS stock_ensamble
        FROM productos
